...
" + consultaInner, conn)
    ...
    Public Sub buscarProyectosDeCliente(ByVal tabla As DataGridView, ByVal idCliente As String)
        Try
            conectar()
            Dim cmd As New SqlCommand(consultaProyetosClientes + " cln.idClient = '" + idCliente + "' ) )as T1)AS T2", conn)
            If cmd.ExecuteNonQuery Then
                Dim da As New SqlDataAdapter(cmd)
                Dim dt As New DataTable
                da.Fill(dt)
    ...
    Dim consultaProyetosClientes As String = "select T2.[Work Order],T2.[Project Description],T2.[Cmp],CONCAT('$',(T2.[Total Amount ST]+T2.[Total Amount OT]+T2.[Total Amount 3]+T2.[Total Expenses Spend]+T2.[Total Material Spend])) as 'Total Spend', 
T2.[Total Hours ST],
...
CONCAT('$',T2.[Total Material Spend]) as 'Total Materual Spend',
...
Or
cln.lastName Like '" + consulta + "' )as T1)AS T2", conn)
            If cmd.ExecuteNonQuery Then
                Dim da As New SqlDataAdapter(cmd)
                Dim dt As New DataTable
                da.Fill(dt)
